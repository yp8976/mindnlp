...
def load_balancing_loss_func(
    router_probs: ms.Tensor, expert_indices: ms.Tensor
) -> float:
    r"""
    ...
    if expert_indices.dtype != ms.int64:
        expert_indices = expert_indices.to(ms.int64)
    ...
    expert_mask = ops.max(expert_mask, axis=-2).values

    # cast to float32 otherwise mean will fail
    expert_mask = expert_mask.to(ms.float32)
...
class NllbMoeSinusoidalPositionalEmbedding(nn.Cell):
...
class NllbMoeTop2Router(nn.Cell):
    """
    ...
    def __init__(self, config: NllbMoeConfig):
        ...
        self.classifier = nn.Dense(
            config.hidden_size, self.num_experts, has_bias=config.router_bias
        )
        self.router_ignore_padding_tokens = config.router_ignore_padding_tokens
        self.dtype = getattr(ms, config.router_dtype)
    ...
    def route_tokens(
        self,
        router_logits: ms.Tensor,
        input_dtype: ms.dtype = ms.float32,
        ...
        router_probs = ops.softmax(router_logits, axis=-1, dtype=self.dtype).to(
            input_dtype
        )
        top_1_expert_index = ops.argmax(router_probs, dim=-1)
        top_1_mask = ops.one_hot(top_1_expert_index, self.num_experts)

        if self.second_expert_policy == "sampling":
            gumbel = nn.probability.distribution.gumbel.Gumbel(0, 1).rsample
        ...
        top_2_mask = ops.one_hot(top_2_expert_index, self.num_experts)
        ...
        if self.second_expert_policy == "random":
            ...
            top_2_mask = top_2_mask * sampled.repeat(self.num_experts, 1).swapaxes(1, 0)
        ...
        if self.batch_prioritized_routing:
            ...
            sorted_cumsum1 = (
                ops.cumsum(sorted_top_1_mask, axis=0) - 1
            ) * sorted_top_1_mask
            ...
            sorted_cumsum2 = (
                ops.cumsum(sorted_top_2_mask, axis=0) - 1
            ) * sorted_top_2_mask
            ...
            locations1 = ops.cumsum(top_1_mask, axis=0) - 1
            locations2 = ops.cumsum(top_2_mask, axis=0) - 1
            # Update 2nd's location by accounting for locations of 1st
            locations2 += ops.sum(top_1_mask, dim=0, keepdim=True)
...
class NllbMoeDenseActDense(nn.Cell):
    def __init__(self, config: NllbMoeConfig, ffn_dim: int):
        super().__init__()
        self.fc1 = nn.Dense(config.d_model, ffn_dim)
        self.fc2 = nn.Dense(ffn_dim, config.d_model)
...
class NllbMoeSparseMLP(nn.Cell):
    r"""
    ...
    def __init__(
        ...
        ffn_dim: int,
        expert_class: nn.Cell = NllbMoeDenseActDense,
    ):
        super().__init__()
    ...
    def construct(
        self, hidden_states: ms.Tensor, padding_mask: Optional[ms.Tensor] = False
    ):
        r"""
        The goal of this construct pass is to have the same number of operation as the equivalent `NllbMoeDenseActDense`
...
class NllbMoeAttention(nn.Cell):
...
class NllbMoeEncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16 and (
            ...
            clamp_value = (
                ms.Tensor(np.finfo(ms.dtype_to_nptype(hidden_states)).max) - 1000
            )
...
class NllbMoeDecoderLayer(nn.Cell):
    def __init__(self, config: NllbMoeConfig, is_sparse: bool = False):
        super().__init__()
        self.embed_dim = config.d_model
        self.is_sparse = is_sparse
    ...
    def construct(
        ...
        Args:
            ...
            cross_attn_layer_head_mask (`ms.Tensor`):
        ...
        if hidden_states.dtype == ms.float16 and ops.isinf(hidden_states).any():
            clamp_value = (
                ms.Tensor(np.finfo(ms.dtype_to_nptype(hidden_states.dtype)).max) - 1000
            )
            hidden_states = ops.clamp(hidden_states, min=-clamp_value, max=clamp_value)
...
class NllbMoeModel(NllbMoePreTrainedModel):
    ...
    def construct(
        ...
        output_router_logits: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], Seq2SeqMoEModelOutput]:
        r"""
        Returns:
...
class NllbMoeForConditionalGeneration(NllbMoePreTrainedModel):
    ...
    def construct(
        ...
        output_router_logits: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], Seq2SeqMoEOutput]:
        r"""
    ...
    def _unpack_router_logits(self, router_outputs):
        ...
        total_router_logits = (
            ops.cat(total_router_logits, axis=1)
            if len(total_router_logits) > 0
            else None
        )
        total_expert_indexes = (
            ops.stack(total_expert_indexes, axis=1)
